function [W, error, count] = my_ANN_momentum(node, M, X, T, alpha, eta)
    % -----参数-----
    % node: 每层节点度数的序列
...
%     eta = 0.08;
    ...
    % 考虑了动量的存在需要每次记录当前一步的权重变化矩阵
    d_W_ori = cell(1, M-1);
    for i = 1:M-1
        d_W_ori{1, i} = zeros(node(i) + 1, node(i+1));
    end
    
    while (error >=epsilon)&&(count <= 5e5)
        ...
        % -----更新权值-----
        for i = 1:M-1
            W{1, i} = W{1, i} + alpha*d_W{1, i} + eta*d_W_ori{1, i};
        end
        % ------------------
        
        % -----替换权值矩阵-----
        for i = 1:M-1
            d_W_ori{1, i} = alpha*d_W{1, i} + eta*d_W_ori{1, i};
        end
%         d_W_ori = d_W;
        % ---------------------
